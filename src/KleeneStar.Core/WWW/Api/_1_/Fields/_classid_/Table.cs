...
{
    /// <summary>
    /// Represents a REST API table for managing fields entities, providing data retrieval 
    /// and option generation functionality for field records.
    /// </summary>
    ...
    public sealed class Table : KleeneStarRestApiTable<Model.Entities.Field>
    {
        ...
        /// the filter statement.
        /// </returns>
        protected override IQuery<Model.Entities.Field> Filter(IEnumerable<string> filters, IQuery<Model.Entities.Field> query, IRequest request)
        {
            foreach (var filter in filters.Where(f => f.StartsWith("qf_", StringComparison.OrdinalIgnoreCase)))
            {
                ...
                {
                    case "active":
                        query = query.Where(x => x.State == FieldState.Active);
                        break;
                    default:
                        continue;
                }
        ...
        /// </returns>
        private static IUri GetUri(Model.Entities.Field row, IRequest request)
        {
            return null;
        }
