...
{
    /// <summary>
    /// Provides CRUD operations for workflow items via a REST API.
    /// </summary>
    [Cache]
    public sealed class Index : global::KleeneStar.Core.WebRestApi.RestApiCrudClassStructure<Model.Entities.Workflow>
    {
        /// <summary>
        ...
        protected override System.Guid? ClassOf(Model.Entities.Workflow item)
        {
            return item?.ClassId;
        }
        ...
        /// </returns>
        protected override IEnumerable<Model.Entities.Workflow> Retrieve(IQuery<Model.Entities.Workflow> query, IQueryContext context, IRequest request)
        {
        ...
        /// </returns>
        protected override IRestApiCrudResultRetrieve RetrieveForClone(IQuery<Model.Entities.Workflow> query, IRequest request)
        {
            using var context = ModelHub.CreateDbContext();
            var data = CoreHub.WorkflowManager.GetWorkflows(query, context)
                .FirstOrDefault();

            var newItem = new Model.Entities.Workflow()
            {
                Name = data.Name + " (Copy)",
        ...
        /// </returns>
        protected override IRestApiCrudResultRetrieve RetrieveForUpdate(IQuery<Model.Entities.Workflow> query, IRequest request)
        {
            using var context = ModelHub.CreateDbContext();
            var data = CoreHub.WorkflowManager.GetWorkflows(query, context)
                .FirstOrDefault();
        ...
        /// </returns>
        protected override IRestApiCrudResultRetrieveDelete RetrieveForDelete(IQuery<Model.Entities.Workflow> query, IRequest request)
        {
            using var context = ModelHub.CreateDbContext();
            var data = CoreHub.WorkflowManager.GetWorkflows(query, context)
                .FirstOrDefault();
        ...
        /// </returns>
        protected override IRestApiCrudResultCreate Create(RestApiCrudFormData fieldMap, IRequest request, out Model.Entities.Workflow newItem)
        {
            var id = Guid.NewGuid();
        ...
        /// </returns>
        protected override IRestApiCrudResultCreate Clone(Model.Entities.Workflow existingItem, RestApiCrudFormData fieldMap, IRequest request, out Model.Entities.Workflow newItem)
        {
            var id = Guid.NewGuid();
            newItem = new Model.Entities.Workflow(id)
            {
        ...
        /// The HTTP request providing additional context.
        /// </param>
        protected override IRestApiCrudResultUpdate Update(Model.Entities.Workflow existingItem, RestApiCrudFormData payload, IRequest request)
        {
        ...
        /// </returns>
        protected override IRestApiCrudResultDelete Delete(Model.Entities.Workflow existingItem, IRequest request)
        {
            CoreHub.WorkflowManager.Remove(existingItem.Id);
