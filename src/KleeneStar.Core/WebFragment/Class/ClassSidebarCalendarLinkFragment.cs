...
{
    /// <summary>
    /// Sidebar item that links to the calendar overview page of the active class.
    /// </summary>
    ...
    [Order(10)]
    [Cache]
    public sealed class ClassSidebarCalendarLinkFragment : FragmentControlSidebarItemLink
    {
        private static readonly IUri _uri = CoreHub.GetUri<global::KleeneStar.Core.WWW.Calendars._classid_.Index>();
        ...
        /// <summary>
        /// Resolves the class id from the active render context, walking through
        /// Calendar/Sla/Form/Workflow parameters when no class parameter is present.
        /// </summary>
        private static Guid ResolveClassId(IRenderControlContext renderContext)
        {
            ...
            var calendarParam = renderContext.Request.GetParameter<CalendarIdParameter>();
            ...
            {
                var calId = Guid.TryParse(calendarParam.Value, out var parsed) ? parsed : Guid.Empty;
                var calendar = CoreHub.CalendarManager.GetCalendar(calId);
                return calendar?.ClassId ?? Guid.Empty;
            }
        ...
        /// <summary>
        /// Returns whether the current request URI points at the calendar overview for this class.
        /// </summary>
        ...
        /// <summary>
        /// Returns the resolved calendar-page URI for the current class.
        /// </summary>
        ...
        /// <summary>
        /// Computes the trailing sidebar badge for the 'calendar' link: the number of calendars
        /// attached to the resolved class. Returns <c>null</c> when no class can be resolved or
        /// the class has no calendars, so an empty collection leaves the link badge-free rather
        /// than showing a noisy zero.
        /// </summary>
        /// <param name="renderContext">
        /// The render context used to resolve the class identifier whose calendars are counted.
        /// Cannot be null.
        /// </param>
        /// <returns>
        ...
        private static string GetBadge(IRenderControlContext renderContext)
        {
            ...
            var count = CoreHub.CalendarManager.GetCalendars(new ClassIdParameter(classId)).Count();
